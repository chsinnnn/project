...
void prettyPrint(ASTNode* node, int currIndent = 0) {
    if (node->type == NodeType::ATOM) {
    	if (systemPrimitives.count(node->value)) {
    		string pri = "#<procedure " + node->value + ">";
            printWithoutNewline(pri);
        }  // if
        else
...
class Evaluator {
	...
	private:
	    ...
	    ASTNode* evalAtom(ASTNode* node) {
			...
			if (systemPrimitives.count(node->value)) {
		        return new ASTNode(NodeType::ATOM, node->value);  // 保留名稱
		    }  // if
	        // 是否是已定義的 symbol
...
//		                cout << "\n  ";
		        ...
		        // 不管 else 有沒有被 define，只有「最後一個」寫 else 才當作語法 else
		        bool isElse = (isLastClause &&
		        ...
		        // 如果是 else，或 eval 結果非 nil → 成立
		        if (isElse || testResult->value != "nil") {
		            if (clause->children.size() == 1) {
		                return isElse ? new ASTNode(NodeType::ATOM, "nil") : testResult;
		            }  // if
					else {
